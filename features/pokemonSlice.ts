...
import { SliceStatus, baseImageUrl } from './../lib/global';
  ...
  reducers: {
    ...
    getPokemonsReducer(
      ...
      const isPokemonAlreadyExists = state.data.find(
        (existingPokemon) => existingPokemon !== null && existingPokemon.id === pokemon.id
      );
      if (!isPokemonAlreadyExists) {
        state.data[state.data.length - (size - index)] = pokemon;
      }
    },
    getSinglePokemonReducer(state, action: PayloadAction<{ pokemon: Pokemon }>) {
