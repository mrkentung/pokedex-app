...
const evolutionChainSlice = createSlice({
  ...
  reducers: {
    ...
    getEvolutionChainReducer(state, action: PayloadAction<{ evolutionChain: EvolutionChain }>) {
...
export const { initialize, error, success, getEvolutionChainReducer } = evolutionChainSlice.actions;
export const statusHandler = { initialize, error, success };

export const evolutionChainSelector = (state: RootState) => state.evolutionChain;
...
export const getEvolutionChainByName = wrapReduxAsyncHandler(
  ...
  async (dispatch, { name }) => {
    ...
    dispatch(getEvolutionChainReducer({ evolutionChain: camelcaseObject(result) }));
  }
);
