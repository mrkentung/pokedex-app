...
import { PokemonTypePlaceholders } from './../lib/global';
...
const PokemonEvolution = ({ pokemon, chain, selectedBackgroundColor }: Props) => {
    ...
    <div className="mb-5 lg:mb-0 lg:flex lg:flex-row w-full">
        ...
        <div className="relative mx-auto h-48 w-full">
          ...
          <div
            ...
            <ProgressiveImage
              ...
              render={(src, style) => <img src={src} style={style} alt={pokemon.name} />}
            />
          </div>
        </div>
        <p className="mt-1 text-sm text-black font-semibold">#{leftPad(pokemon.id, 3)}</p>
      ...
      <p className="flex items-center mx-auto mt-2 mb-4 lg:mb-0 lg:mt-0">
        {chain?.evolvesTo.length !== 0 && (
          <>
            <AiOutlineCaretDown className="block mx-auto lg:hidden opacity-75" size={24} />
            <AiOutlineCaretRight className="hidden mx-auto lg:block opacity-75" size={24} />
          </>
        )}
      </p>
    </div>
