...
const trans2 = (x: number, y: number) => `translate3d(${x / 20}px,${y / 20}px,0)`;
...
const PokemonDetailsHeader = ({ pokemon, species, selectedBackgroundColor }: Props) => {
  ...
  const kanjiName = species.names.find((name) => name.language.name === 'ja-Hrkt');
    ...
    <>
      <div
        ...
        <div className="px-4 md:px-8">
          <p className="text-md mt-4 text-white font-medium">#{leftPad(pokemon.id, 3)}</p>
        ...
        <div className="relative text-center mx-auto w-full h-96 mt-8 lg:mt-24">
          ...
          <animated.div
            ...
            <ProgressiveImage
              ...
              render={(src, style) => <img src={src} alt={pokemon.name} style={style} />}
            />
          </animated.div>
        </div>
      </div>
