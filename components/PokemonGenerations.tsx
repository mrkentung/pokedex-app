...
import { importPokemonImage } from './../lib/global';
...
const generations = [
  ...
  [importPokemonImage('treecko'), importPokemonImage('torchic'), importPokemonImage('mudkip')],
  [importPokemonImage('turtwig'), importPokemonImage('chimcar'), importPokemonImage('piplup')],
  [importPokemonImage('snivy'), importPokemonImage('tepig'), importPokemonImage('oshawott')],
  [importPokemonImage('chespin'), importPokemonImage('fennekin'), importPokemonImage('froakie')],
  [importPokemonImage('rowlet'), importPokemonImage('litten'), importPokemonImage('popplio')],
];

type Props = {
  selectedGeneration: PokemonGenerationsEnum | null;
  setSelectedGeneration: React.Dispatch<React.SetStateAction<PokemonGenerationsEnum | null>>;
      ...
      <Modal.Button
        ...
        <div className="flex justify-between">
          {selectedGeneration !== null ? (
            <>
              {generations[pokemonGenerationsToIndex(selectedGeneration)].map((image, index) => (
                <PokemonIcon key={`${image}-${index}`} src={image} alt={`Pokemon Icon Image`} />
              ))}
            </>
          ) : (
            <>
              <PokemonIcon src={importPokemonImage('bulbasaur')} alt="Bulbasaur" />
              <PokemonIcon src={importPokemonImage('charmander')} alt="Charmander" />
              <PokemonIcon src={importPokemonImage('squirtle')} alt="Squirtle" />
            </>
          )}
        </div>
      </Modal.Button>
      <Modal.Content title="Pokémon Generations" handleSaveModal={changeGenerationHandler}>
