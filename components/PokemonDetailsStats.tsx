  ...
  return (
    <>
      ...
      <ul className="capitalize">
        {stats.map((st) => (
          <PokemonStats key={`stats-${st.name}`} title={st.name} min={st.min} max={st.max} />
        ))}
      ...
      <p className="mt-10 mb-6 text-darkerGray font-medium">
        Min & Max values are calculated for level 100 Pokemon. Minimum values are based on 0 EVs & 0
        IVs, meanwhile Maximum values are based on 252 EVs & 31 IVs.
      </p>
    </>
  );
};
